...
def n2means(samples, mask):
    n = 5
    #print('Idividual: ', indi2)
    #indi2 += 1
    new_samples = []
    cdef int num_thteads, j
    cdef int tam
    for sample in samples:
        new_sample = []
        tam = len(samples[0])
...
def main():
    ...
    for g in range(n_generations):
        ...
        for i in range(n_individuals):
            results.append(n2means(samples,gen))
        
        print("recalculando as gerações")
        for ind in range(n_individuals):
            count = gen[ind][1:].count(1)
            gen[ind][0] = results[ind] + count * alpha
        
        print("ordenando os melhores")
