...
def dist(sample, c):
    return abs(operator.sub(sample[1:], c[1:]))
...
@cuda.jit
def centroid(samples, cs, sums, counts, dists, mindist, arr0, arr1):
    ind1 = 0
    ind2 = 0
    for sample in samples:
            ...
            c = cs[0]
            c_l = c[1]
            op = abs(operator.sub(ord(sample_l[1:]), ord(c_l[1:])))
            dists[0] = op
            ...
            for i in sums[mindist]:
                i = operator.add(i, ord(sample[1]))

            #sums[mindist] = operator.add(sums[mindist], sample[1])
            counts[mindist] += 1
    return tuple(tuple(arr0, arr1), tuple(sums, counts))

#@cuda.jit
def kmeans(samples, k):
    csi = list(np.random.choice(range(len(samples)), size=k*2, replace=False))
    cs = samples[csi[0]], samples[csi[1]]
    dists = [dist(samples[0][1], c[1]) for c in cs]
    ...
    while different_cs(cs, prev_cs):
        ...
        arr0 = [samples[0] for i in range(len(samples[0]))]
        arr1 = [samples[0] for i in range(len(samples[0]))]

        #gs = [i for i in range(k)]
        sums = [np.array([0 for j in range(len(samples[0][1]))]) for i in range(k)]
        counts = [0 for i in range(k)]
        blockspergrid_x = math.ceil(len(sums) / threadsperblock[0])
        blockspergrid_y = math.ceil(len(sums[0]) / threadsperblock[1])
        blockspergrid = (blockspergrid_x, blockspergrid_y)


        #kkk = centroid[blockspergrid, threadsperblock](samples, cs, sums, counts, dists, mindist, arr0, arr1)
        
        #arr0, arr1, sums, counts
        ...
        prev_cs = deepcopy(cs)
        #div[blockspergrid, threadsperblock](cs,sums,counts)
        #div(cs,sums,counts)
