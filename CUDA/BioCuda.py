...
class Bio(nn.Module):
    ...
    def dist(self, sample, c):
        return np.linalg.norm(np.subtract(sample[1:], c[1:]))


    def eval_group(self, groups):
    ...
    def evalprint_group(self, group):
        labels = {'AML': 0, 'ALL': 0}
        for sample in group:
            labels[sample[0]] += 1
    ...
    def kmeans(self,samples, k):
        ...
        gs = [[] for i in range(k)]
        while self.different_cs(cs, prev_cs):
    ...
    def n2means(self, samples, mask):
        ...
        best_model = None
        for i in range(n):
            groups = self.kmeans(new_samples, 2)

            e0, e1 = self.eval_group(groups)
...
for g in range(n_generations):
    ...
    print('Generation: ', g)
    #pool = ThreadPool(4)
    results = IA.n2means(samples, gen)
